...
1. **Java 17 JDK** (OpenJDK 17 or Eclipse Temurin LTS)
2. **Maven 3.x** (Note: A local Maven distribution is already included in the project directory `maven/` if running from original source files)
3. **MySQL Server** (or a local stack like WampServer / XAMPP / Laragon)
4. **Mantra MFS500 Fingerprint Scanner** (with physical hardware drivers installed)
...
3. Verify that the correct version is active in your terminal with:
   ```powershell
   java -version
   ```
...
To run with a physical fingerprint scanner, you must activate the SDK license. Run the included `setup_licensing.bat` script as an Administrator to configure the local licensing service and launch the Activation Wizard:
```powershell
.\setup_licensing.bat
```
...
1. Open the configuration file `application.properties` in: `src/main/resources/application.properties`.
...
You can run the application either using **Visual Studio Code** or via the **Command Line (Maven)**.

#### Option 1: Run via Visual Studio Code (Recommended)
...
3. **Run the Project**:
   ...
   * Alternatively, open the `Main.java` file and click the inline **Run** button that appears directly above the `main` method.
...
If you prefer running via terminal, navigate to the project root directory and use the following commands:

* **Using globally installed Maven**:
  ```powershell
  # Compile the project
  mvn clean compile
...
* **Using the locally bundled Maven** (if running from the original folder containing the `maven/` directory):
  ```powershell
  # Compile the project
